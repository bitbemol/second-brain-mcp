...
| **Broad search is bounded and does not create a disclosure bypass** | `search_vault` enumerates only supported textual files under `notes/`, skips hidden, packaged, symlinked, non-regular, unsupported, and not-currently-downloaded ubiquitous entries, then resolves every candidate through the existing contained target boundary. Each note is opened without following raced path components, snapshotted under a shared path lease, and read through a stable descriptor with a byte ceiling no larger than the request's remaining aggregate budget; a non-fitting file is counted and skipped without aborting later candidates. Matching happens after release and returns no mutation revision. A bounded cancellation-aware in-process queue and a vault-scoped cross-process permit prevent concurrent scans from multiplying the corpus ceiling. Queries are literal data—never regular expressions or subprocess arguments—and query/prefix collections, traversal, aggregate bytes, Markdown lines/front matter/tags, sections, source tokens, general and fuzzy comparisons, edit-distance cells, candidates, metadata, snippets, and the complete MCP wire response are capped. Responses distinguish omitted matching results from incomplete search coverage and report known resource-limited files separately. Canvas searches only projected node values. HAR is sanitized before matching; every other textual snapshot and every returned field passes the sensitive-content policy. Unsafe files contribute only bounded skip counts, not values or query-dependent diagnostics. Structured MCP values and JSON encoding keep snippets as untrusted strings. |
