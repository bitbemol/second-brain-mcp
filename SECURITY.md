...
| **Broad search is bounded and does not create a disclosure bypass** | `search_vault` enumerates only supported textual files under `notes/`, skips hidden, packaged, symlinked, non-regular, unsupported, and not-currently-downloaded ubiquitous entries, then resolves every candidate through the existing contained target boundary. Each note is opened without following raced path components, snapshotted under a shared path lease, and read through a stable descriptor with an 8 MiB search-specific and request-wide aggregate ceiling; ordinary notes and smaller files are admitted before large HAR evidence, and a non-fitting file is counted without aborting later candidates. Smart matching completes a corpus-wide literal pass before fair per-document relaxed work, so one archive cannot consume later notes' comparison allowance. Matching happens after release and returns no mutation revision. A bounded cancellation-aware in-process queue and a vault-scoped cross-process permit prevent concurrent scans from multiplying the corpus ceiling. Queries are literal data—never regular expressions or subprocess arguments—and query/prefix collections, traversal, aggregate bytes, Markdown lines/front matter/tags, sections, source tokens, general and fuzzy comparisons, edit-distance cells, candidates, metadata, snippets, diagnostics, and the complete MCP wire response are capped. Responses distinguish omitted matching results from incomplete search coverage and expose only a bounded sensitive-policy-validated sample of resource-limited paths. Canvas searches only projected node values. HAR is sanitized before matching; every other textual snapshot and every returned field passes the sensitive-content policy. Unsafe files contribute only bounded skip counts, not values or query-dependent diagnostics. Structured MCP values and JSON encoding keep snippets as untrusted strings. |
