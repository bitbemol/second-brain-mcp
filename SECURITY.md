...
| **Concurrent MCP clients cannot interleave note mutations** | Fair actor-based reader/writer leases coordinate tasks in one runtime; persistent shared/exclusive advisory locks enforce the same path exclusion across independent MCP processes. OS record-lock polling does not guarantee strict cross-process FIFO order. A vault-wide process lock also protects the shared Git index and commit sequence. Ordinary reference reads remain concurrent; expensive direct PDF snapshot/PDFKit reads share a bounded local queue and vault-scoped cross-process permit. |
| **Broad search is bounded and does not create a disclosure bypass** | `search_vault` enumerates only registered textual formats under `notes/` and PDFs under `references/`; it skips hidden, packaged, symlinked, non-regular, unsupported, and not-currently-downloaded ubiquitous entries, then resolves every candidate through the existing contained target boundary. Files are opened without following raced path components, and search-mode descriptor walks reject hidden flags on every opened vault descendant. Snapshots use shared path leases and stable descriptors with 8 MiB text, 64 MiB PDF, a 128 MiB raw-snapshot aggregate ceiling, a 64 MiB retained-projection ceiling, and a 100,000-section aggregate ceiling for decompressed/structured projections. PDFKit receives only immutable bounded PDF data; page count and extracted-text bytes are capped, page objects drain inside per-page autorelease pools, and broad search never performs implicit OCR. Ordinary notes and smaller files are admitted before large HAR/PDF evidence, and a non-fitting file is counted without aborting later candidates. Smart matching completes a corpus-wide whole-literal pass before fair per-document relaxed work, so one archive cannot consume later files' comparison allowance. Matching happens after release and returns no mutation revision. A bounded cancellation-aware in-process queue and a vault-scoped cross-process permit prevent concurrent scans from multiplying the corpus ceiling. Queries are literal data—never regular expressions or subprocess arguments—and query/prefix collections, traversal, aggregate bytes, Markdown lines/front matter/tags, sections/pages, source tokens, literal occurrences, general and fuzzy comparisons, edit-distance cells, candidates, metadata/locators, snippets, cursors, diagnostics, and the complete MCP wire response are capped. Continuation cursors are bound to admitted file revisions and projection outcomes and fail closed if either changes between pages. Responses distinguish omitted matching results from incomplete coverage, expose bounded sensitive-policy-validated resource samples, and report PDF extraction/OCR status. Canvas searches only projected node values. HAR is sanitized before matching; extracted PDF text, other textual snapshots, and every returned field pass the sensitive-content policy. Unsafe files contribute only bounded skip counts, not values or query-dependent diagnostics. Structured MCP values and JSON encoding keep all returned vault data untrusted. |
