...
Only `notes/` and `references/` need to exist. Writable startup connects the MCP transport before recovering pending note changes, so initialization and tool discovery do not wait behind a large or contended snapshot. Recovery holds a shared vault lease: reads and discovery remain available, and a mutation arriving during the one in-flight recovery attempt waits at the normal exclusive vault boundary. Startup recovery is attempted once and is never restarted by tool calls. Every mutation first snapshots its validated one- or two-path pre-change footprint under the exclusive lease; failure leaves persistence untouched and returns `SNAPSHOT_FAILED` with `state: not_applied`. A successful preflight is followed by persistence and the required post-change snapshot; only a failure after persistence starts has an unconfirmed outcome. Recursive directory moves preflight and reconcile the complete notes tree. Recovery and transport completion are logged to stderr without Git arguments, paths, status text, or stderr. On input EOF, SIGTERM, SIGINT (Ctrl-C), or SIGHUP (terminal closure), the server closes tool admission, cancels outstanding calls, and waits for their work to unwind; persistence that has already started still finishes its required snapshot. Catchable termination signals disconnect the transport instead of immediately exiting, so startup-recovery cancellation can terminate and reap its Git child while the parent is still alive. SIGKILL, a process crash, and machine failure cannot run this cleanup; a client that force-kills the server before its drain completes can still leave an orphan child. Read-only startup neither initializes snapshots nor resolves Git.
...
Live recovery verifies the stable device/inode identity of its private per-vault data root throughout each snapshot, including around every Git child, and opens its lock below captured no-follow root and lock-directory descriptors. It safely tightens an existing current-user-owned repository through a descriptor-relative no-follow open back to mode `0700`. A legitimate repair therefore does not require a restart solely because it was created with ordinary directory permissions, while replaced ancestors, links, foreign-owned paths, unsafe internal layouts, and corrupt objects fail closed. The selected Apple Git executable is revalidated between snapshot attempts; if it can no longer launch, the failed attempt discards that cached path so the next recovery attempt resolves and validates Git again.
...
Full recovery snapshots force inclusion of every exact regular-file byte under `notes/`; interactive file mutations force the exact bytes of their validated changed paths. Both are independent of user ignore and attribute rules. Symbolic links, special filesystem entries, and nested Git repositories fail closed when they are inside the snapshot scope; unrelated entries cannot stall an interactive one-file mutation. The product accepts only a canonical Apple-signed Git executable found in the selected developer directory, Xcode, Xcode beta, or Command Line Tools; `/usr/bin/git` shims are rejected. Upgrading from the former user-repository snapshot design creates a private baseline from the current `notes/` state and leaves all old user refs/history untouched. Moving or copying a vault to another canonical path creates a separate private baseline because private storage is keyed by that path.

Startup recovery deliberately examines the complete `notes/` corpus rather than trusting cached file metadata, so it can capture changes left by an interrupted process or another local app. Interactive file mutations do not repeat that work: they snapshot only their validated changed paths against the latest private tree, so a one-file update does not read or hash unrelated work-tree files, including slow cloud-backed content. Loading the private baseline index still scales with its path count. A later startup recovery reconciles any direct external edits outside those paths. Recursive directory moves retain full-tree reconciliation cost. Keep editor caches, generated data, and bulky non-note content outside `notes/`; exceptionally large or slow/cloud-backed trees can still reach the cooperative 120-second boundary during full recovery.
...
The matrix and each format's create input and update modes come from one exhaustive backend catalog. Input schemas use explicit top-level object properties without root `oneOf`, `anyOf`, or `allOf`, keeping the same eight tool names and argument names across clients. Conditional requirements are documented in tool/parameter descriptions and enforced by the server before persistence, rather than relying on provider support for conditional schemas. Catalog-derived mode descriptions specify supported format/mode combinations: `replace` and `append` require `content`; `patch` requires 1–20 `replacements` objects containing `old_text` and `new_text`, and forbids `content`. MCP tool discovery derives format enums and descriptions from that catalog; `VaultFileService` enforces the registered create/update contracts. Simpler discovery schemas do not weaken runtime validation; provider acceptance must still be verified in the client's model integration. Adding a format does not require a resource or a second frontend registry. Internal handler identities stay private. In `--read-only` mode, mutating tools disappear from discovery.
