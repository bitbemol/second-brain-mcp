...
│   ├── AttachmentManager.swift # actor — list + soft-delete attachments (non-note/canvas files under notes/, .trash, git)
...
│   ├── VideoImporter.swift      # actor — import an external video into notes/ as an animated GIF (sample frames + assemble GIF, git)
│   ├── VideoEncoding.swift     # protocol — platform seam for video inspect/GIF-assembly (inspect is async)
│   ├── AVFoundationVideoEncoder.swift # macOS AVFoundation+ImageIO impl of VideoEncoding (#if canImport(AVFoundation))
...
Tests/SecondBrainMCPTests/      # PathValidator (exhaustive — the security backbone),
                                ...
                                # Canvas (model + manager), ImageManager (+ encoder), LinkResolver, ImageImporter, VideoImporter.
```

**Concurrency:** actors for mutable state + I/O (VaultManager, GitManager, AuditLogger, ImageImporter, VideoImporter, AttachmentManager);
...
- **Links resolve by *basename across the whole vault*, not relative to the note's folder.** `LinkResolver` mirrors Obsidian: `![[screenshot.png]]` in `apple/uikit/foo.md` resolves to `apple/_attachments/screenshot.png` — the bare basename is matched vault-wide (extension-less targets imply a `.md` note; path-qualified targets match by path; ties broken by proximity to the `from` note, then shortest path). Don't "fix" this into folder-relative resolution — it would break the embeds Obsidian actually follows. `find_backlinks` **resolves each candidate link before counting**, so two files sharing a basename don't cross-contaminate each other's backlinks. **Safety:** user input (a link target / file ref) is only ever *string-matched against the `VaultEnumerator` candidate list* — never used to open a path — so there's no traversal surface and no `PathValidator` call is needed here. **Known limitation:** link extraction is regex-based and does *not* skip fenced/inline code, so a `[[link]]` shown as an example inside a ``` block counts as a reference — acceptable for now (a markdown-aware skip would be the fix if it ever bites).
...
- **`add_image` is the *only* tool that reads a path *outside* the vault sandbox** — and that's deliberate, not a hole. The source path is arbitrary (a screenshot the agent produced); the **image decode in `ImageEncoding.inspect` is the security gate on it** — a renamed script/archive/non-image won't decode and is rejected before any write. The output is **re-encoded to a clean PNG** (decode→re-encode), so EXIF, trailing bytes, and polyglot/appended payloads never reach the vault — this is why `add_image` chose re-encode over a verbatim copy. The **destination is still fully `PathValidator`-gated** under `notes/` (reject-if-exists, extension normalized to `.png`). So the "one path gate" rule still holds for the *write* side; the source side is gated by image-validity instead. The source is **canonicalized (symlinks resolved) before any check**, then required to be a **regular file** (no FIFO/device → the decoder can't stall the actor), **size-capped on the resolved target** (a symlink can't hide a file past the cap), and **refused if it resolves inside the vault** (`add_image` imports *external* files only). The decode is raced against a **30 s timeout in the handler** (like `read_image`) so a crafted/corrupt source can't hang the import. **The source is only ever *read* — never moved, deleted, or modified.** The server deliberately does *not* manage external filesystem state; its entire write/delete footprint is inside the vault, so `add_image` is purely additive (`destructiveHint: false`, reject-if-exists). Removing a vault image is a separate in-domain op (`delete_attachment` → soft-delete to `.trash/`). **Residual, accepted:** it can still read any image the process can access and copy it in — the decode gate stops *non*-images, but a source-directory allowlist (config) is the lever if stricter control is ever wanted.
...
- **`add_video` converts in-process with AVFoundation + ImageIO — never ffmpeg / `Process()`.** It's the video twin of `add_image`: it samples a source video to frames and assembles an **animated GIF** stored under `notes/`, *because* `read_image` already turns an animated GIF into a timed PNG frame bundle — so the model can "watch" a recording, and the GIF plays in Obsidian. AVFoundation & ImageIO are system frameworks (same trust surface as the existing PDFKit/ImageIO code), so this stays within Rule 4 — **don't reach for ffmpeg or a third `Process()` site.** The platform work lives behind `VideoEncoding` (`AVFoundationVideoEncoder`, `#if canImport(AVFoundation)`); `VideoImporter` is pure policy, unit-tested with a fake encoder. **`VideoEncoding.inspect`/`makeGIF` are `async`** (unlike `ImageEncoding`'s sync methods): AVFoundation loads metadata and renders frames asynchronously (`AVURLAsset.load`, `AVAssetImageGenerator.image(at:)`), and awaiting them beats blocking a thread on a semaphore. The **decode is the gate** on the external source (same as `add_image`): a file with no video track is rejected before any write; the source is canonicalized, must be a regular file, size-capped on the resolved target, and **refused if it resolves inside the vault** — `add_video` imports *external* files only and only ever **reads** the source. Frame schedule (pure, unit-tested `frameSchedule`): `frameCount = min(ceil(duration×fps), maxFrames)`, evenly spaced across `[0, duration)`, `frameDelay = duration/frameCount` (playback ≈ real-time) — short clip = full fps, long clip spreads the cap across the whole video. **Balanced caps** (`VideoImporter.Config`, internal like `ImageManager.Config`): fps 10 · long edge 1080 · maxFrames 120 · source ≤ 512 MB · duration ≤ 30 min · **output GIF ≤ 50 MB**. That last one is *the real guard* — GIFs don't inter-frame compress, so the assembled output is size-checked and rejected (with guidance to shorten / lower fidelity) before it's written; the source cap is just a coarse pre-filter. The handler races the conversion against a **120 s timeout** (wider than `read_image`'s 30 s — transcoding many frames is heavier).
